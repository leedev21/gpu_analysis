...
class BatchSampler():
    def __init__(self, test_case, config, seed):
        ...
        self.process_groups = {}
        self.memory = {'in': 0}
        self.config_as_memory_check = False
        self.memory_not_match = False
        self.create()
    ...
    @staticmethod
    def _tensor_memory(shape, dtype, device):
        if device != 'cpu' and device != torch.device('cpu'):
            return math.prod(shape) * 2 # default use f16
        return 0
    ...
    def rand_tensor(self, data, dtype, device, args=None, strides=None, distributions=None, k_tensor=None):
        self.memory['in'] += self._tensor_memory(data, dtype, device)
        if self.config_as_memory_check:
            return None
    ...
    def create(self):
        ...
        if self.check_memory(dtype, device):
            ...
            self.memory_not_match = True

    def check_memory(self, dtype, device, min=0, max=20):
        self.config_as_memory_check = True
        if self.by_new_style:
            self.random_data_new(self.test_case.input, dtype, device, self.config)
        else:
            self.random_data(self.test_case.input, dtype, device, self.config)
        ...
        self.args = []
        self.kwargs = {}
        if self.get_memory() < min or self.get_memory() > max:
            return False
        return True

    def get_memory(self):
        return float(self.memory['in']) / 1024 / 1024 / 1024
