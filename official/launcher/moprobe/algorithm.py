...
from .compare_utils import ULP_PARAMETERS, CompareConst
...
def get_ulp_err(bench_output, device_output, dtype):
    all_with_fp32 = True
    if draw_manager.ulp_dtype:
        dtype = draw_manager.ulp_dtype
    parameters = ULP_PARAMETERS.get(dtype)
    min_eb = parameters.get('min_eb')[0]
    mantissa_num = parameters.get('mantissa_num')[0]
    abs_bench = np.abs(bench_output)
    eb = np.where(abs_bench == 0, 0, np.floor(np.log2(abs_bench)))
    eb = np.maximum(eb, min_eb)

    if dtype == torch.float32 and not all_with_fp32:
        ulp_err = calc_ulp_err(bench_output, device_output, eb, mantissa_num, np.float64)
    else:
        ulp_err = calc_ulp_err(bench_output, device_output, eb, mantissa_num, np.float32)
    if draw_manager.draw_ulp:
        line = str(ulp_err)[:50] + ' ,..., ' + str(ulp_err)[-50:]
        print('ulp_err:', dtype, min_eb, mantissa_num, line.replace('\n', ';'))      # default print the ulp err.
        print("Min ULP error:", np.min(ulp_err), "Max ULP error:", np.max(ulp_err))
        draw_manager.add_tensor('ulp_err', dtype, [eb, ulp_err])
    ulp_err = np.abs(ulp_err)
    return ulp_err, dtype


def calc_ulp_err(bench_output, device_output, eb, mantissa_num, data_type):
    return (device_output.astype(data_type) - bench_output).astype(data_type) * \
            np.exp2(-eb + mantissa_num).astype(data_type)
