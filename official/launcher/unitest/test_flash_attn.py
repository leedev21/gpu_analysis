...
try:
    ...
    from flash_attn.flash_attn_interface import _get_block_size as _get_block_size_n
...
def generate_qkv(
    ...
    if query_padding_mask is not None:
        q_unpad, indices_q, cu_seqlens_q, max_seqlen_q = unpad_input(q, query_padding_mask) # flash_attn2.4
    ...
    if key_padding_mask is not None:
        k_unpad, indices_k, cu_seqlens_k, max_seqlen_k = unpad_input(k, key_padding_mask) # flash_attn2.4
        v_unpad, _, _, _ = unpad_input(v, key_padding_mask) # flash_attn2.4
    else:
...
@pytest.mark.parametrize("seed", [1234, 200, 42, 0, 9999, 2023])
@pytest.mark.parametrize('dropout_p', [0.0])
@pytest.mark.parametrize("softcap", [0.0])
def test_flash_attn_varlen_output(seed, seqlen_q, seqlen_k, d, dropout_p, causal, local, alibi, deterministic, mha_type, dtype, kvpacked, softcap
):
    if (
    ...
    assert torch.allclose(out, out_ref, rtol=3.9e-3, atol=3.9e-3)
    # assert torch.allclose(out, out_ref, rtol=3.9e-3, atol=1e-5)
    ...
    if (d <= MAX_HEADDIM_SM8x or dropout_p == 0) or (is_sm80 or is_sm90):
        ...
        print("++++++++++++++++++++++++++++++++++++++++++++")
        assert torch.allclose(dq, dq_ref, rtol=3.9e-3, atol=3.9e-3)
        assert torch.allclose(dk, dk_ref, rtol=6e-3, atol=6e-3)
        assert torch.allclose(dv, dv_ref, rtol=6e-3, atol=6e-3)
