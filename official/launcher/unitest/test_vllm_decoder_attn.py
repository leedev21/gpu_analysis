...
rotary_emb = get_rope(qk_rope_head_dim,
                    ...
                    rope_scaling=None,
                    is_neox_style=False)
q_b_proj = torch.nn.Linear(q_lora_rank,
                                num_attention_heads *
                                qk_head_dim,
                                bias=False,)
kv_b_proj = torch.nn.Linear(
            ...
            bias=False,)
o_proj = torch.nn.Linear(num_attention_heads * v_head_dim,
                            hidden_size,
                            bias=False)
...
test_dec_attn(16, 64, _Backend.XFORMERS, 1, 16, 128)