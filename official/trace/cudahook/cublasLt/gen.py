...
#include "common/trace_profile.h"
...
HOOK_C_API HOOK_DECL_EXPORT $ret$ $func_name$($func_param$) {
    auto flag = VALUE(CH_LOG_ENABLE_CUBLASLT);     
    using func_ptr = $ret$ (*)($param_type$);
    int64_t enable = 0;
    if (hook::Get_$func_name$_Ptr() == hook::Get_$func_name$_NativePtr()){
     enable = 0 == (flag&1) ? 0 : 1;        
    } else {
     enable = 0 == (flag&2) ? 0 : 1;        
    }
    HOOK_TRACE_PROFILE(enable, "$func_name$");
