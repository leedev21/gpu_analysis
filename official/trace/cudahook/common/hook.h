...
class Handle {
 ...
 public:
  Handle(const char *so) {
    HLOGEX("load_so %s", so);
    handle_ = dlopen(so, RTLD_NOW | RTLD_LOCAL);
    HOOK_CHECK(handle_);
  }
  ~Handle() = default;
...
#define HOOK_GET_SYMBOL(type, symbol_name)                    \
  do {                                                        \
    static Handle h_##type(s_##type##_dso);                   \
    auto ret = dlsym(h_##type.handle(), symbol_name.c_str()); \
    HOOK_CHECK(ret);                                          \
    return ret;                                               \
  } while (0)
...
class Hook {
 public:
  ...
  static void *GetCUPTISymbol(const std::string &symbol_name) {
    HOOK_GET_SYMBOL(cupti, symbol_name);
  }
  static void *GetNCCLSymbol(const std::string &symbol_name) {
    HOOK_GET_SYMBOL(nccl, symbol_name);
  }

 private:
  // nvidia native cuda dynamic library can be modified to any other unambiguous
  // name, or moved to any path
  static constexpr const char *s_cuda_dso =
      "/usr/lib/x86_64-linux-gnu/libcuda.so";
  static constexpr const char *s_nvml_dso =
      "/usr/lib/x86_64-linux-gnu/libnvidia-ml.so.1";
  static constexpr const char *s_cudart_dso =
      "/usr/local/cuda/targets/x86_64-linux/lib/libcudart.so";
  static constexpr const char *s_cudnn_dso =
      "/usr/lib/x86_64-linux-gnu/libcudnn.so.9";
  static constexpr const char *s_cublas_dso =
      "/usr/local/cuda/targets/x86_64-linux/lib/libcublas.so";
  static constexpr const char *s_cublasLt_dso =
      "/usr/local/cuda/targets/x86_64-linux/lib/libcublasLt.so";
  static constexpr const char *s_cufft_dso =
      "/usr/local/cuda/targets/x86_64-linux/lib/libcufft.so";
  static constexpr const char *s_nvtx_dso =
      "/usr/local/cuda/targets/x86_64-linux/lib/libnvToolsExt.so";
  static constexpr const char *s_nvrtc_dso =
      "/usr/local/cuda/targets/x86_64-linux/lib/libnvrtc.so";
  static constexpr const char *s_curand_dso =
      "/usr/local/cuda/targets/x86_64-linux/lib/libcurand.so";
  static constexpr const char *s_cusparse_dso =
      "/usr/local/cuda/targets/x86_64-linux/lib/libcusparse.so";
  static constexpr const char *s_cusolver_dso =
      "/usr/local/cuda/targets/x86_64-linux/lib/libcusolver.so";
  static constexpr const char *s_nvjpeg_dso =
      "/usr/local/cuda/targets/x86_64-linux/lib/libnvjpeg.so";
  static constexpr const char *s_nvblas_dso =
      "/usr/local/cuda/targets/x86_64-linux/lib/libnvblas.so";
  static constexpr const char *s_cupti_dso =
      "/usr/local/cuda/targets/x86_64-linux/lib/libcupti.so";
  static constexpr const char *s_nccl_dso =
...
#define HOOK_CUPTI_SYMBOL(symbol_name) Hook::GetCUPTISymbol(symbol_name)
...
#define HOOK_NCCL_SYMBOL(symbol_name) Hook::GetNCCLSymbol(symbol_name)
