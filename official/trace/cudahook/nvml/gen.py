...
class CodeGenerate():
    def __init__(self, type_, file_, output_):
        ...
        self.func_list = []
        self.black_list = ['nvmlDeviceGetGpuFabricInfoV', 'nvmlSystemGetConfComputeSettings']

        #self.hook_file = self.output + "/" + self.type + "_hook.cpp"
        self.hook_file = self.output + "/" + os.path.basename(file_) + "_hook.cpp"        
...
"""
        self.hook_template = """
namespace hook{
class $func_name$_PtrProvider {
 private:
  void* raw_;
  void* hook_;

 public:
  $func_name$_PtrProvider(void* h) {      
    cudaHookInit();
    if (0 == VALUE(CH_WORK_MODE)) {
      raw_ = HOOK_$type$_SYMBOL("$func_name$");        
    }
    hook_ = h;
  }
  ~$func_name$_PtrProvider() = default;
  void* Get() { return hook_ ? hook_ : raw_; }
  void* GetRaw() { return raw_; }
};
$func_name$_PtrProvider* Get_$func_name$_Obj(void* hook = nullptr) {
  static $func_name$_PtrProvider instace(hook);
  return &instace;
}
void* Get_$func_name$_NativePtr() {
  return Get_$func_name$_Obj()->GetRaw();
}
void* Get_$func_name$_Ptr(void* hook = nullptr) {
  return Get_$func_name$_Obj(hook)->Get();
}
}
HOOK_C_API HOOK_DECL_EXPORT $ret$ $func_name$($func_param$) {
    static bool log_enable = !isApiInBlackList("$func_name$");
    using func_ptr = $ret$ (*)($param_type$);
    static auto func_entry = reinterpret_cast<func_ptr>(hook::Get_$func_name$_Ptr());
    HOOK_CHECK(func_entry);
    auto ret = func_entry($param_name$);
    if (log_enable) {    
        ParamDumper inst(__func__);
        inst.Dump($param_name$);
    }
    //CHECK_RETURN(ret);  
    return ret;
}
"""
    ...
    def generate_func(self):
        for func in self.header.functions:
            func_name = func["name"]
            if func["static"]:
                #print("static  %r" %func_name)
                continue
            if func["inline"]:
                #print("inline  %r" %func_name)
                continue              
            if func["extern"]:
                #print("extern  %r" %func_name)
                continue
            if func_name in self.black_list:
                continue
            if func_name in self.func_list:
                continue
            else:
                self.func_list.append(func_name)
                #print ("append  %r" %func_name)
    ...
    def save_output(self):
        ...
        with open(self.hook_file, 'w') as fh:
            hook_include = self.hook_include.replace("$hook_num$", str(
                len(self.hook_list))).replace("$header$", os.path.basename(self.file))
